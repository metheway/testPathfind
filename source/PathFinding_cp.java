...
public class PathFinding_cp {
	...
	public PathFinding_cp() {	//CONSTRUCTOR
		clearMap();
		initialize();
	}
		...
		public ArrayList<Node> sortQue(ArrayList<Node> sort) {	//SORT PRIORITY QUE
			...
			while(c < sort.size()) {
				...
				for(int i = c+1; i < sort.size(); i++) {
					// f小的排在前面
					double f1 =  sort.get(i).getEuclidDist() + sort.get(i).getHops();
					double f2 = sort.get(sm).getEuclidDist() + sort.get(sm).getHops();
					...
					if (f1 < f2) {
						...
						if (sort.get(i).getHops() > sort.get(sm).getHops()) {
							sm = i;
						}
					}
